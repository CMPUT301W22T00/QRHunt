...
public class Player {
    ...
    public Boolean isAdmin(Player possibleAdmin){
        return possibleAdmin.admin;
    }

    public String deletePlayer(Player bannedPlayer, Player adminBanning){
        if (isAdmin(adminBanning)){
    ...
    public String makeAdmin(Player newAdmin, Player approvingAdmin){
        if (isAdmin(approvingAdmin)){
            newAdmin.admin = true;
            return "Player now admin.";
        } else {
            newAdmin.admin = false;
            return "Player did not become admin.";
        }
    }
